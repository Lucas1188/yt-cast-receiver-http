...
import HttpPlayer, { type FakeState } from './HttpPlayer.js';
import HttpPlayerDemoLogger from './HttpPlayerDemoLogger.js';
import HttpPlayerDemoScreen from './ui/HttpPlayerDemoScreen.js';
import { hostname } from 'os';

/**
 * Demo application showing usage of the `yt-cast-receiver` library.
 */
class HttpPlayerRuntime {

  #player: Player;
  #screen: HttpPlayerDemoScreen | null;
  #logger: Logger;
  ...
  constructor() {
    ...
    if(process.argv.includes('--port')){
      port = parseInt(process.argv[process.argv.indexOf('--port')+1])|| port
    }
    // Create our own player instance
    const player = this.#player = new HttpPlayer(port);

    // Create UI (if not disabled)
    const screen = this.#screen = !noUI ? new HttpPlayerDemoScreen() : null;
    ...
    const screenLogger = screen ? new HttpPlayerDemoLogger(screen.logBox) : null;

    // To clear stored context
    const defaultDS = new DefaultDataStore()
    if(process.argv.includes('--clear-ds')){
        Promise.resolve(defaultDS.clear())
    }
    var name = process.argv.includes('--name')? process.argv[process.argv.indexOf('--name')+1] : hostname()
    ...
    const receiver = this.#receiver = new YouTubeCastReceiver(player, {
      ...
      logLevel: 'info', // Ouput debug messages
      dataStore: defaultDS
    });
    ...
    receiver.on('error', (error) => {
      this.#logger.error('[HttpPlayer] Error occurred:', error);
    });
  ...
  #registerKeyPressListener() {
    ...
    this.#screen.on('keypress', (ch, key: {ch: string, name: string, shift: boolean, ctrl: boolean}) => {
      ...
      if (this.#screen.logLevelWindow.visible) {
        ...
        if (targetLogLevel) {
          this.#logger.info(`[HttpPlayer] Setting log level to: ${targetLogLevel}`);
  ...
  #handleQueueEvent(event: PlaylistEvent) {
    let msg: string | null = '[HttpPlayer] ';
  ...
  async start() {
    try {
      ...
      this.#logger.error('[HttpPlayer] Error occurred while starting receiver:', error);
      return;
    }
    this.#logger.info('[HttpPlayer] YouTubeCastReceiver started.');
    ...
    this.#pairingCodeRequestService.on('request', () => {
      ...
      this.#logger.debug('[HttpPlayer] Obtaining code for manual pairing...');
    });
    this.#pairingCodeRequestService.on('response', (code) => {
      // `response` event: pairing code obtained
      this.#logger.info(`[HttpPlayer] Code for manual pairing (aka Link with TV code): ${code}`);
    });
    this.#pairingCodeRequestService.on('error', (error) => {
      // Service automatically stops on `error` event
      this.#logger.error('[HttpPlayer] Error occurred while obtaining code for manual pairing:', error);
    });
  ...
  async stop() {
    try {
      ...
      this.#logger.error('[HttpPlayer] Error occurred while stopping receiver:', error);
      return;
    }

    this.#pairingCodeRequestService.removeAllListeners();
    this.#pairingCodeRequestService.stop();
    this.#logger.info('[HttpPlayer] YouTubeCastReceiver stopped.');
  }
  ...
  async exit() {
    try {
      ...
      this.#logger.warn('[HttpPlayer] Error occurred while stopping receiver. Exiting uncleanly...');
    }
    this.#logger.info('[HttpPlayer] Bye!');
    process.exit(0);
  }
}

const rt = new HttpPlayerRuntime();
void rt.start();
